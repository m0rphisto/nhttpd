...
const
   ...
   {log, collect} = require('@lib/Logger'),
...
////////////////////////////////////////////////////////////////////////////////
// TEMPORARY: TODO build a little Date module !!!!
 ...
 */
const eMailTemplate = ({ email, name, organization, title, message }) => {
   return `email: ${email}
name: ${name || '*****'}
...
const chkHeaders = (headers) => {
   ...
   const whitelist = Load.json('HTTPHeadersList.json').map((regex) => new RegExp(regex, 'i'));
...
exports.chkInput = async (req, res) => {
   ...
   if (! allowedMethods.includes(req.method)) {
      log(1, cfg.ipaddr, `Found forbidden request method ${req.method}`);
   ...
   if (URL && (forbidden(URL) || !allowed(URL) || !isSafePath(URL))) {
      ...
      log(1, cfg.ipaddr, `${status_codes['404']} FORBIDDEN ${reg.method} ${req.url}`);
      return !!0;
   }
   ...
   if (hasDoubleEncoding(req.url)) {
      ...
      res.writeHead(404, header(`${cfg.HOSTNAME}:${cfg.PORT}`, 'txt'));
      res.end(`${status_codes['404']}`);
      log(1, cfg.ipaddr, `${status_codes['404']} (~XSS~) ${req.method} ${req.url}`);
      return !!0;
   }
   ...
   for (const [key, value] of Object.entries(req.headers)) {
      collect(key, cfg.ipaddr, value);
      if (key in cfg.httpHeaders) continue;
      log(1, cfg.ipaddr, `BLOCKED HEADER ${req.method} ${key}(${value})`);
   ...
   if (req.method === 'POST') {
      ...
      const file = `inquiry.${d.timestamp()}.${cfg.ipaddr}.txt`;
      ...
      fs.writeFile(path.join(dir, file), eMail, { encoding: 'utf8', flag: 'w', mode: 0o0640 }, (err) => {
         if (err) {
            log(1, cfg.ipaddr, `ERROR Could not save contact form - ${err}`);
            cfg.postData.txt = `Could not save contact form. Please contact webmaster@${cfg.HOSTNAME}`;
            cfg.postData.code = '500';
         }
         if (suspicious || !valid) {
            log(1, cfg.ipaddr, `ERROR ${req.method} Saved suspicious/invalid input!`);
            res.writeHead(400, header(`${cfg.HOSTNAME}:${cfg.PORT}`, 'txt'));

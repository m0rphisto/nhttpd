...
exports.data = () => {
   ...
   const header = Template.parse(view, {
      'HEADER_TITLE': '.m0rph\'s blog :: Security',
   ...
   const files = cl.getFiles(path.join(cfg.ROOT, 'views', 'blog', 'security'));
   files.sort((a, b) => {return b.mtime - a.mtime}); // b - a := sort descending (otherwise a - b)
   // Version 1.0 loads complete all articles in a section. Later on, when the number of articles grows
   // we will implement an indexing and searching function, respectivele NUM_POST limitation.
